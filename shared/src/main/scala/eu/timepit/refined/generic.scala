...
private[refined] trait GenericPredicates {
  ...
  implicit def fieldNamesPredicate[T, P, R <: HList, K <: HList](implicit lg: LabelledGeneric.Aux[T, R],
    keys: Keys.Aux[R, K], ktl: ToList[K, Any], p: Predicate[P, List[String]]): Predicate[FieldNames[P], T] = {

    val fieldNames = keys().toList.map(_.toString.drop(1))
    Predicate.instance(_ => p.isValid(fieldNames), _ => p.show(fieldNames))
  }
